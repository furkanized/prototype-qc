import { r as registerInstance, c as createEvent, h, F as Fragment, d as getElement, a as Host } from './index-D4ALLk7q.js';
import { d as dateFnsExports } from './index-O1A-ZSZs.js';
import { c as classNames } from './index-D7YaJBkB.js';
import { C as ClickOutsideMixin } from './clickoutside-mixin-C0U9wSBw.js';
import { f as floatingElementAutoUpdate } from './position-utils-BiHzhysf.js';
import { g as getDataTestId } from './test-id-utils-BDMnqEGL.js';
import { v as v4 } from './v4-CK3_k8jD.js';
import { g as getIconElementProps } from './icon-utils-SXopALR1.js';
...
const TkDatePicker = class {
    constructor(hostRef) {
        ...
        this.tkInvalid = createEvent(this, "tk-invalid", 7);
        ...
        this.handleApply = () => {
            ...
            if (this.timeOnly) {
                if (this.internalStartTime) {
                    ...
                    emitValue = dateFnsExports.format(tempDate, this.getOnlyTimeFormat());
                }
            }
            else {
                ...
                if (this.inputValue) {
                    if (this.timeOnly) {
                        ...
                        if (parsedTime) {
                            ...
                            this.isInvalid = false;
                            if (!this.allowApplyButton) {
                                this.tkChange.emit(dateFnsExports.format(parsedTime, this.getOnlyTimeFormat()));
                            }
                        }
                        ...
                        if (parsedDate && !this.isDateDisabled(parsedDate)) {
                            ...
                            if (this.showTimePicker) {
                                ...
                                formattedValue = dateFnsExports.format(parsedDate, this.getFullDateTimeFormat());
                            }
                            else {
                                this.internalStartTime = null;
                                this.internalEndTime = null;
    ...
    measureCalendarTableHeight() {
        var _a;
        const table = (_a = this.panelRef) === null || _a === void 0 ? void 0 : _a.querySelector('.tk-datepicker-table');
    ...
    formatDateOrDateTime(date, type) {
        if (this.timeOnly) {
            ...
            return dateFnsExports.format(temp, this.getOnlyTimeFormat());
        }
    ...
    parseTimeString(timeString) {
        ...
        let parsed = dateFnsExports.parse(timeString, primaryFmt, base);
        if (dateFnsExports.isValid(parsed) && dateFnsExports.format(parsed, primaryFmt) === timeString) {
            return parsed;
        }
        // Fallbacks
        ...
        if (this.timeFormat === '12') {
            ...
            const alt24 = dateFnsExports.parse(timeString, 'HH:mm', base);
            if (dateFnsExports.isValid(alt24) && dateFnsExports.format(alt24, 'HH:mm') === timeString) {
                return alt24;
            }
        }
        return null;
    ...
    emitTimeChange() {
        ...
        if (this.timeOnly) {
            ...
            const temp = new Date();
            temp.setHours(this.internalStartTime.hour, this.internalStartTime.minute, 0, 0);
            const value = dateFnsExports.format(temp, this.getOnlyTimeFormat());
    ...
    createDayCell(date, isAdjacentMonth) {
        ...
        return (h("td", { class: classNames('tk-datepicker-day-cell', {
                'selected': isSelected,
                'in-range': isInRange,
            ...
            }), onClick: () => !isDisabled && this.handleDateClick(date), onMouseEnter: () => this.handleDateHover(date), "data-testid": getDataTestId(this.dataTestid, 'day-cell', dateKey) }, h("span", { class: "tk-datepicker-day", "data-testid": getDataTestId(this.dataTestid, 'day', dateKey) }, date.getDate())));
    }
    createWeekDays() {
        ...
        for (let i = 1; i <= daysInCurrentMonth; i++) {
            ...
            if (days.length === 7) {
                weeks.push(h("tr", { "data-testid": getDataTestId(this.dataTestid, 'week', weeks.length.toString()) }, days));
                days = [];
            }
        }
        // Next month's days
        ...
        return (h("tbody", { class: "tk-datepicker-days", "data-testid": getDataTestId(this.dataTestid, 'days') }, weeks));
    }
    createWeekDayNames() {
        ...
        return (h("thead", { "data-testid": getDataTestId(this.dataTestid, 'week-days-header') }, h("tr", { class: "tk-datepicker-week-days", "data-testid": getDataTestId(this.dataTestid, 'week-days-row') }, weekdays.map((day, index) => (h("th", { class: "tk-datepicker-week-day-cell", "data-testid": getDataTestId(this.dataTestid, 'week-cell', index.toString()) }, h("span", { class: "tk-datepicker-week-day", "data-testid": getDataTestId(this.dataTestid, 'week-day', index.toString()) }, day)))))));
    }
    createMonths() {
        ...
        for (let i = 0; i < 12; i += 4) {
            ...
            rows.push(h("tr", { class: "tk-datepicker-month-row", "data-testid": getDataTestId(this.dataTestid, 'month', (i / 4).toString()) }, monthGroup.map((month, index) => {
                const monthIndex = i + index;
                const isSelected = this.currentMonth.getMonth() === monthIndex;
                return (h("td", { class: classNames('tk-datepicker-month', {
                        selected: isSelected,
                    }), "data-testid": getDataTestId(this.dataTestid, 'month-option', monthIndex.toString()), onClick: (e) => {
                        e.stopPropagation();
        ...
        return (h("tbody", { class: "tk-datepicker-months", "data-testid": getDataTestId(this.dataTestid, 'months') }, rows));
    }
    createYears() {
        ...
        for (let i = 0; i < years.length; i += 4) {
            ...
            rows.push(h("tr", { class: "tk-datepicker-year-row", "data-testid": getDataTestId(this.dataTestid, 'year', (i / 4).toString()) }, yearGroup.map(year => (h("td", { class: classNames('tk-datepicker-year', {
                    selected: year === currentYear,
                }), "data-testid": getDataTestId(this.dataTestid, 'year', year.toString()), onClick: e => this.handleYearSelect(e, year) }, year)))));
        }
        return (h("tbody", { class: "tk-datepicker-years", "data-testid": getDataTestId(this.dataTestid, 'years') }, rows));
    }
    createHeader() {
        ...
        return (h("div", { class: headerClasses, "data-testid": getDataTestId(this.dataTestid, 'header') }, h("div", { class: "tk-datepicker-header-content", "data-testid": getDataTestId(this.dataTestid, 'header-content') }, h("div", { class: "tk-datepicker-header-content-start", "data-testid": getDataTestId(this.dataTestid, 'header-start') }, h("tk-button", { variant: this.headerType === 'primary' || this.headerType === 'dark' ? 'white' : 'neutral', icon: "keyboard_double_arrow_left", "onTk-click": () => this.handleYearChange(-1), type: "text", disabled: this.readonly || this.disabled || this.loading, dataTestid: getDataTestId(this.dataTestid, 'prev-year-button') }), h("span", { class: "tk-datepicker-divider", "data-testid": getDataTestId(this.dataTestid, 'header-start-divider') }), h("tk-button", { variant: this.headerType === 'primary' || this.headerType === 'dark' ? 'white' : 'neutral', icon: "chevron_left", "onTk-click": () => this.handleMonthChange(-1), type: "text", disabled: this.readonly || this.disabled || this.loading, dataTestid: getDataTestId(this.dataTestid, 'prev-month-button') })), h("div", { class: "tk-datepicker-select-container", "data-testid": getDataTestId(this.dataTestid, 'header-select') }, h("div", { class: classNames('tk-datepicker-select-month', { disabled: this.readonly || this.disabled || this.loading }), onClick: e => !this.loading && this.handleViewChange(e, 'months'), "data-testid": getDataTestId(this.dataTestid, 'header-month') }, monthName), h("div", { class: classNames('tk-datepicker-select-year', { disabled: this.readonly || this.disabled || this.loading }), onClick: e => !this.loading && this.handleViewChange(e, 'years'), "data-testid": getDataTestId(this.dataTestid, 'header-year') }, year)), h("div", { class: "tk-datepicker-header-content-end", "data-testid": getDataTestId(this.dataTestid, 'header-end') }, h("tk-button", { variant: this.headerType === 'primary' || this.headerType === 'dark' ? 'white' : 'neutral', icon: "chevron_right", "onTk-click": () => this.handleMonthChange(1), type: "text", disabled: this.readonly || this.disabled || this.loading, dataTestid: getDataTestId(this.dataTestid, 'next-month-button') }), h("span", { class: "tk-datepicker-divider", "data-testid": getDataTestId(this.dataTestid, 'header-end-divider') }), h("tk-button", { variant: this.headerType === 'primary' || this.headerType === 'dark' ? 'white' : 'neutral', icon: "keyboard_double_arrow_right", "onTk-click": () => this.handleYearChange(1), type: "text", disabled: this.readonly || this.disabled || this.loading, dataTestid: getDataTestId(this.dataTestid, 'next-year-button') })))));
    }
    createFooter() {
        if (this.hasFooterSlot) {
            ...
            return (h("div", { class: footerClass, "data-testid": getDataTestId(this.dataTestid, 'footer') }, h("slot", { name: "footer-actions" })));
        }
        return null;
    }
    createTimePicker() {
        ...
        return (h("div", { class: classNames('tk-datepicker-timepicker-panel', this.timeOnly && 'tk-datepicker-timepicker-panel-only'), "data-testid": getDataTestId(this.dataTestid, 'timepicker') }, h("div", { class: classNames('tk-datepicker-timepicker-header', `tk-datepicker-timepicker-header-${this.headerType}`, this.timeOnly && 'tk-datepicker-timepicker-header-only'), "data-testid": getDataTestId(this.dataTestid, 'timepicker-header') }, this.timeFormat === '12' && (h("tk-toggle-button-group", { value: this.internalAmPm, type: this.headerType === 'basic' ? 'basic' : this.headerType === 'light' ? 'light' : 'divided', "onTk-change": e => this.handleAmPmToggle(e), class: "tk-datepicker-ampm-toggle", dataTestid: getDataTestId(this.dataTestid, 'timepicker-toggle') }, h("tk-toggle-button", { key: "AM", type: this.headerType === 'primary' || this.headerType === 'dark' ? 'filled' : 'text', variant: this.headerType === 'dark' ? 'neutral' : 'primary', value: "AM", label: "AM", size: "small", disabled: isDisabled || AMDisabled, dataTestid: getDataTestId(this.dataTestid, 'timepicker-am') }), h("tk-toggle-button", { key: "PM", type: this.headerType === 'primary' || this.headerType === 'dark' ? 'filled' : 'text', variant: this.headerType === 'dark' ? 'neutral' : 'primary', value: "PM", label: "PM", size: "small", disabled: isDisabled || PMDisabled, dataTestid: getDataTestId(this.dataTestid, 'timepicker-pm') })))), h("div", { class: classNames('tk-datepicker-timepicker-body', ['primary', 'light', 'dark'].includes(this.headerType) && `tk-datepicker-timepicker-body-${this.headerType}`, this.weeksLength === 4 && 'tk-datepicker-timepicker-body-4-weeks', this.timeOnly && 'tk-datepicker-timepicker-body-only'), style: {
                height: this.calendarTableHeightPx ? `${this.calendarTableHeightPx}px` : undefined,
            }, "data-testid": getDataTestId(this.dataTestid, 'timepicker-body') }, h("div", { class: "tk-datepicker-timepicker-col", "data-testid": getDataTestId(this.dataTestid, 'timepicker-hours') }, h("div", null, h("tk-button", { variant: this.headerType === 'primary' || this.headerType === 'dark' ? 'white' : 'neutral', type: "text", size: "base", icon: "expand_less", "onTk-click": this.handleDecreaseHour, disabled: isMinHour || prevHourDisabled || isDisabled, dataTestid: getDataTestId(this.dataTestid, 'timepicker-hours-decrease') }), h("div", { class: classNames('tk-datepicker-timepicker-separator', {
            ...
            }), "data-testid": getDataTestId(this.dataTestid, 'timepicker-hours-separator-top') })), visibleHours.map((hour, hourIndex) => hour === null ? (h("div", { class: classNames('tk-datepicker-timepicker-value tk-datepicker-timepicker-value-empty', { disabled: isDisabled }), "data-testid": getDataTestId(this.dataTestid, 'timepicker-hours-value-empty', hourIndex.toString()) })) : (h("div", { class: classNames('tk-datepicker-timepicker-value', {
            ...
            }), "data-testid": getDataTestId(this.dataTestid, 'timepicker-hour', hour.toString()), onClick: () => {
            ...
            } }, hour.toString().padStart(2, '0')))), h("div", null, h("div", { class: classNames('tk-datepicker-timepicker-separator', {
                'tk-datepicker-timepicker-separator-dark': this.headerType === 'dark',
                'tk-datepicker-timepicker-separator-primary': this.headerType === 'primary',
            }), "data-testid": getDataTestId(this.dataTestid, 'timepicker-hours-separator-bottom') }), h("tk-button", { variant: this.headerType === 'primary' || this.headerType === 'dark' ? 'white' : 'neutral', type: "text", size: "base", icon: "expand_more", "onTk-click": this.handleIncreaseHour, disabled: isMaxHour || nextHourDisabled || isDisabled, dataTestid: getDataTestId(this.dataTestid, 'timepicker-hours-increase') }))), h("div", { class: "tk-datepicker-timepicker-col", "data-testid": getDataTestId(this.dataTestid, 'timepicker-minutes') }, h("div", null, h("tk-button", { variant: this.headerType === 'dark' || this.headerType === 'primary' ? 'white' : 'neutral', type: "text", size: "base", icon: "expand_less", "onTk-click": this.handleDecreaseMinute, disabled: isMinMinute || prevMinuteDisabled || isDisabled, dataTestid: getDataTestId(this.dataTestid, 'timepicker-minutes-decrease') }), h("div", { class: classNames('tk-datepicker-timepicker-separator', {
                'tk-datepicker-timepicker-separator-dark': this.headerType === 'dark',
                'tk-datepicker-timepicker-separator-primary': this.headerType === 'primary',
            }), "data-testid": getDataTestId(this.dataTestid, 'timepicker-minutes-separator-top') })), visibleMinutes.map((m, minuteIndex) => m === null ? (h("div", { class: classNames('tk-datepicker-timepicker-value tk-datepicker-timepicker-value-empty', { disabled: isDisabled }), "data-testid": getDataTestId(this.dataTestid, 'timepicker-minutes-value-empty', minuteIndex.toString()) })) : (h("div", { class: classNames('tk-datepicker-timepicker-value', {
                'selected': m === currentMinute,
                'tk-datepicker-timepicker-value-dark': this.headerType === 'dark',
                'tk-datepicker-timepicker-value-primary': this.headerType === 'primary',
                'disabled': isDisabled || isMinuteDisabled(m),
            }), "data-testid": getDataTestId(this.dataTestid, 'timepicker-minute', m.toString()), onClick: () => {
                if (isDisabled || isMinuteDisabled(m))
                    return;
                this.handleMinuteClick(m);
            } }, String(m).padStart(2, '0')))), h("div", null, h("div", { class: classNames('tk-datepicker-timepicker-separator', {
                'tk-datepicker-timepicker-separator-dark': this.headerType === 'dark',
                'tk-datepicker-timepicker-separator-primary': this.headerType === 'primary',
            }), "data-testid": getDataTestId(this.dataTestid, 'timepicker-minutes-separator-bottom') }), h("tk-button", { variant: this.headerType === 'dark' || this.headerType === 'primary' ? 'white' : 'neutral', type: "text", size: "base", icon: "expand_more", "onTk-click": this.handleIncreaseMinute, disabled: isMaxMinute || nextMinuteDisabled || isDisabled, dataTestid: getDataTestId(this.dataTestid, 'timepicker-minutes-increase') }))))));
    }
    createLoading() {
        return (h("div", { class: "tk-datepicker-loading", "data-testid": getDataTestId(this.dataTestid, 'loading') }, h("tk-spinner", { size: this.size, dataTestid: getDataTestId(this.dataTestid, 'loading-spinner') })));
    }
    renderInput() {
        if (this.inline)
            return null;
    ...
    renderPanel() {
        ...
        if (this.timeOnly) {
            return (h("div", { class: panelClasses, ref: el => (this.panelRef = el), role: !this.inline ? 'dialog' : undefined, "aria-modal": "true", "data-tk-datepicker-id": this.uniqueId, "data-testid": getDataTestId(this.dataTestid, 'panel') }, h("div", { class: "tk-datepicker-panel-inner", "data-testid": getDataTestId(this.dataTestid, 'panel-inner') }, this.createTimePicker()), this.createFooter()));
        }
        // Default: date (calendar), optionally alongside time picker
        return (h("div", { class: panelClasses, ref: el => (this.panelRef = el), role: !this.inline ? 'dialog' : undefined, "aria-modal": "true", "data-tk-datepicker-id": this.uniqueId, style: { visibility: this.concealUntilMeasured ? 'hidden' : undefined }, "data-testid": getDataTestId(this.dataTestid, 'panel') }, h("div", { class: "tk-datepicker-panel-inner", "data-testid": getDataTestId(this.dataTestid, 'panel-inner') }, h("div", { class: "tk-datepicker-calendar-container", "data-testid": getDataTestId(this.dataTestid, 'calendar') }, this.createHeader(), h("div", { class: bodyClasses, "data-testid": getDataTestId(this.dataTestid, 'body') }, this.loading ? (this.createLoading()) : (h("table", { class: "tk-datepicker-table", "data-testid": getDataTestId(this.dataTestid, 'table') }, this.currentView === 'days' && (h(Fragment, null, this.createWeekDayNames(), this.createWeekDays())), this.currentView === 'months' && this.createMonths(), this.currentView === 'years' && this.createYears())))), this.showTimePicker && this.createTimePicker()), this.createFooter()));
    }
    render() {
        return (h("div", { key: '80b177ebfad0d28bc21c37aaa1991158c83cfc26', class: "tk-datepicker-container", "data-testid": getDataTestId(this.dataTestid, 'container') }, this.renderInput(), this.renderPanel()));
    }
...
const TkPagination = class {
    ...
    createPageNumbers() {
        return this.getPageNumbers().map(pageNumber => {
            ...
            return (h("button", { class: classNames('tk-pagination-cell tk-pagination-page', { 'tk-pagination-page-active': pageNumber === this.internalCurrentPage }), type: "button", onClick: () => this.handlePageClick(pageNumber) }, h("span", { class: "tk-pagination-page-label" }, pageNumber)));
        });
    }
    renderTag(totalPages) {
        if (this.mode !== 'compact') {
            ...
            if (this.mode === 'compact-expanded') {
                ...
                tagContent = h("span", { class: "tk-pagination-tag-label" }, itemsText);
            }
            else {
                const pageText = formatTemplate(this.pageReportTemplate, templateValues);
                const itemsText = formatTemplate(this.itemsReportTemplate, templateValues);
                tagContent = totalPages > 0 && (h(Fragment, null, h("span", { class: "tk-pagination-tag-label" }, pageText), h("span", { class: "tk-pagination-tag-label" }, itemsText)));
            }
    ...
    renderContent(totalPages) {
        ...
        if (this.mode === 'compact') {
            ...
            content = (h(Fragment, null, h("button", { class: "tk-pagination-cell tk-pagination-prev", type: "button", onClick: this.handlePrevClick, disabled: this.internalCurrentPage === 1 }, h("tk-icon", Object.assign({}, getIconElementProps('chevron_left', { color: 'var(--icon-base)' })))), h("tk-input", { mode: "text", value: this.inputValue, "onTk-change": (event) => this.handlePageInputChange(event.detail.toString()), "onTk-blur": () => this.validateAndUpdatePage(), min: 1, max: totalPages }), h("span", { class: "tk-pagination-current-label" }, compactLabel), h("button", { class: "tk-pagination-cell tk-pagination-next", type: "button", onClick: this.handleNextClick, disabled: this.internalCurrentPage === totalPages }, h("tk-icon", Object.assign({}, getIconElementProps('chevron_right', { color: 'var(--icon-base)' }))))));
        }
        ...
        // }
            ...
            content = (h(Fragment, null, h("div", { class: "tk-pagination-prev-actions" }, h("button", { class: "tk-pagination-cell tk-pagination-first", type: "button", onClick: () => this.handlePageClick(1), disabled: this.internalCurrentPage === 1 }, h("tk-icon", Object.assign({}, getIconElementProps('keyboard_double_arrow_left', { color: 'var(--icon-base)' })))), this.type === 'grouped' && h("span", { class: "tk-pagination-divider" }), h("button", { class: "tk-pagination-cell tk-pagination-prev", type: "button", onClick: this.handlePrevClick, disabled: this.internalCurrentPage === 1 }, h("tk-icon", Object.assign({}, getIconElementProps('chevron_left', { color: 'var(--icon-base)' }))), ' ')), this.createPageNumbers(), h("div", { class: "tk-pagination-next-actions" }, h("button", { class: "tk-pagination-cell tk-pagination-next", type: "button", onClick: this.handleNextClick, disabled: this.internalCurrentPage === totalPages }, h("tk-icon", Object.assign({}, getIconElementProps('chevron_right', { color: 'var(--icon-base)' })))), this.type === 'grouped' && h("span", { class: "tk-pagination-divider" }), h("button", { class: "tk-pagination-cell tk-pagination-last", type: "button", onClick: () => this.handlePageClick(totalPages), disabled: this.internalCurrentPage === totalPages }, h("tk-icon", Object.assign({}, getIconElementProps('keyboard_double_arrow_right', { color: 'var(--icon-base)' })))))));
        }
        return h(ContentWrapperTag, { class: contentClasses }, content);
    }
    renderSelect() {
        return (h("tk-select", { style: { width: '75px' }, value: this.rowsPerPage, options: this.rowsPerPageOptions, "onTk-change": e => {
    ...
    renderInput(totalPages) {
        return (h("tk-input", { style: { width: '70px' }, mode: "text", min: 1, max: totalPages, value: this.inputValue, icon: "chevron_right", iconPosition: "right", "onTk-change": (event) => {
                this.handlePageInputChange(event.detail.toString());
            }, "onTk-blur": () => this.validateAndUpdatePage() }));
    }
    render() {
        const totalPages = this.getTotalPages();
        return (h("div", { key: 'ffc6159bb2ead21b8e793abe3e72abc5c33a62bb', class: "tk-pagination-container" }, h("div", { key: '64c521213434bc4a3164f0a04cddc7237a8b3e98', class: "tk-pagination-start" }, this.renderTag(totalPages)), this.renderContent(totalPages), h("div", { key: 'c26ec2ec063ccbd2c3f84c6c6eb92ea27ee801c0', class: "tk-pagination-end" }, this.renderSelect(), this.renderInput(totalPages))));
    }
    static get watchers() { return {
        "internalCurrentPage": [{
...
const TkRadio = class {
    ...
    render() {
        ...
        return (h(Host, { key: '92262fceb575a73daaccdaec498a70dd9073ede3', "data-tk-radio-id": this.uniqueId, invalid: this.invalid, disabled: this.disabled }, h("label", { key: 'e2f683494e14e42f333b449037471d6f0f24e865', htmlFor: this.uniqueId, class: labelClass, "aria-disabled": this.disabled, "aria-invalid": this.invalid }, h("input", { key: '7a15a9f05c0ee1c08dc95803404caa1ee6008d59', id: this.uniqueId, type: "radio", name: this.name, value: this.value, checked: this.checked, disabled: this.disabled, onChange: () => this.handleInputChange() }), h("div", { key: 'bc1d16c1d7007e67848a67ed652931ca4b218db7', class: "mask" }, h("div", { key: '282040082c5e42618c7bf7a28f7d43d088ef8dbf' })), this.hasContentSlot ? (h("slot", { name: "content" })) : (h("div", { class: "tk-radio-text-holder" }, this.label && h("div", { class: "tk-radio-label" }, this.label), this.description && h("div", { class: "tk-radio-description" }, this.description))))));
    }
    static get formAssociated() { return true; }
    get el() { return getElement(this); }
};
...
const TkTreeView = class {
    constructor(hostRef) {
        ...
        this.tkChange = createEvent(this, "tk-change", 7);
        this.tkExpandChange = createEvent(this, "tk-expand-change", 7);
        ...
        this.renderItem = (item, basePath = '', index, depth = 0) => {
            ...
            return (h("div", { class: nodeClass }, this.showPointer && (isExpanded || isSelected) && h("span", { class: classNames('tk-tree-view', 'pointer', this.size) }), h("div", { class: classNames('tk-tree-view', 'label', {
                    selected: isSelected,
                    disabled: isDisabled,
                }, this.size), onClick: () => {
                    this.handleItemClick(pathStr, item, isDisabled, isDirectory);
                } }, isDirectory && this.mode === 'basic' && h("tk-icon", { variant: isSelected ? 'primary' : 'neutral', icon: isExpanded ? 'arrow_drop_down' : 'arrow_right', size: this.size }), this.selectable && (h("tk-checkbox", { onClick: e => {
                    e.stopPropagation();
                ...
                } })), isDirectory && this.branchIcon && h("tk-icon", { icon: this.branchIcon, variant: isSelected ? 'primary' : 'neutral', size: this.size }), !isDirectory && this.leafIcon && h("tk-icon", { icon: this.leafIcon, variant: isSelected ? 'primary' : 'neutral', size: this.size }), h("div", { class: classNames('tk-tree-view', 'text-container', this.size) }, h("span", { class: classNames('tk-tree-view', 'text', this.size) }, item.label), (() => {
                ...
                return (shouldShowBadge && (h("tk-badge", { count: badgeCount, size: this.size, type: (_c = (_b = this.badgeOptions) === null || _b === void 0 ? void 0 : _b.type) !== null && _c !== void 0 ? _c : 'filledlight', variant: (_e = (_d = this.badgeOptions) === null || _d === void 0 ? void 0 : _d.variant) !== null && _e !== void 0 ? _e : 'neutral', rounded: (_g = (_f = this.badgeOptions) === null || _f === void 0 ? void 0 : _f.rounded) !== null && _g !== void 0 ? _g : true, icon: (_h = this.badgeOptions) === null || _h === void 0 ? void 0 : _h.icon, iconPosition: (_j = this.badgeOptions) === null || _j === void 0 ? void 0 : _j.iconPosition })));
            })()), this.mode === 'stepper' && isDirectory && item.children && item.children.length > 0 && (h("tk-icon", { variant: isSelected ? 'primary' : 'neutral', icon: !isExpanded ? 'keyboard_arrow_down' : 'keyboard_arrow_right', size: this.size }))), this.mode === 'basic' && isDirectory && isExpanded && item.children && item.children.length > 0 && (h("div", { class: classNames('tk-tree-view', 'children') }, item.children.map((child, childIndex) => this.renderItem(child, pathStr, childIndex, depth + 1))))));
        };
    }
    itemsChanged() {
    ...
    renderSelectAll() {
        ...
        return (h("div", { class: classNames('select-all', this.size, { disabled: this.disabled }), onClick: this.handleSelectAllClick }, h("div", { class: classNames('select-all-content') }, h("tk-checkbox", { value: this.isAllSelected, indeterminate: isIndeterminate, disabled: this.disabled, onClick: e => e.stopPropagation(), "onTk-change": e => {
                e.stopPropagation();
                this.handleSelectAllClick();
            } }), h("div", null, this.selectAllLabel)), selectedCount > 0 && h("tk-badge", { count: selectedCount, size: this.size, type: "filledlight", variant: "neutral", rounded: true })));
    }
    render() {
        ...
        if (this.mode === 'stepper') {
            ...
            return (h("div", { class: classNames('tk-tree-view', 'stepper'), style: this.containerStyle }, this.renderSelectAll(), columns.map((column, idx) => (h("div", { class: classNames('tk-tree-view', 'column', 'divided'), key: idx, style: this.stepStyle }, column.items.map((item, itemIndex) => this.renderItem(item, column.basePath, itemIndex)))))));
        }
        else {
            const rootClasses = classNames('tk-tree-view', this.type, { disabled: this.disabled });
            return (h("div", { class: rootClasses, style: this.containerStyle }, this.renderSelectAll(), h("div", { class: classNames('tk-tree-view', 'content') }, this.items.map((item, index) => this.renderItem(item, '', index)))));
        }
    }
    get el() { return getElement(this); }
    static get watchers() { return {
...
export { TkDatePicker as tk_datepicker, TkPagination as tk_pagination, TkRadio as tk_radio, TkTreeView as tk_tree_view };
